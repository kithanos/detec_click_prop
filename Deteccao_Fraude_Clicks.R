...
# Autor: Lucas Kitano
# Notas: Projeto com Feedback 1 do curso Big Data Analytics com R e Microsoft Azure Machine Learning da
# formação Cientista de Dados da Data Science Academy "https://www.datascienceacademy.com.br/"


# Carrega Pacotes Necessários
# Use o comando "install.packages("nome_do_pacote") para instalar algum dos pacotes abaixo, caso não o possua.
library(sqldf)
library("ggplot2")
library("dplyr")
library(ROSE)
...
# Carrega o dataset, como o dataset original é muito grande (~7,4 GB), foi utilizado o comando "read_csv.sql"
# que permite carregar um arquivo csv aplicando uma instrução da linguagem SQL, que no caso foi utilizada
# para trazer amostras 1.000.000 de amostras randmomicas do dataset original. Infelizmente este comando
# não possui um parâmetro de controle (seed), compromentendo a total reproducibilidade do experimento,
# entretanto, a performance do modelo aqui criado não deve diferir muito das obtidas em qualquer execução
# do script.
...
# Carrega o dataset
#dados <- read.csv(file = arquivo, header = TRUE, sep = ",", stringsAsFactors = F)
#dados <- fread(file = arquivo, header = TRUE, sep = ',', stringsAsFactor = F, showProgress = TRUE)
...
# As variáveis ip, app, device, os, and channel estão codificadas.
...
ggplot(data = dados, aes(x = factor(is_attributed, labels = c("Download não Realizado", "Download Realizado")))) + 
  geom_bar() + 
  ggtitle("Proporção Variável Target") + xlab("") + theme(plot.title = element_text(hjust = 0.5))

#?ggtitle
#?ggplot

#barplot(prop.table(table(dados$is_attributed)), names.arg = c("Download não Realizado", "Download Realizado"), main = "Prop. Variável Target")
#?barplot

# A distribuição de valores da variável alvo estão desbalanceadas, há apenas 0,2~0,3% de registros no dataset de casos em que 
# o app foi baixado após o clique na propaganda, sendo assim, iremos aplicar posteriormente o balançeamento de classes. 


# Verificando se temos valores ausentes no dataset
sum(is.na(dados))

# Normalmente não consideramos as variáveis de ID no modelo, no entanto, como o endereço IP identifica
# a localidade do usuário, vamos agrupar o endereço IP por faixas de valores.
...
# Podemos observar que para os cliques que não geraram downloads, a maioria estão na localidade L01-L03, enquanto
# que para os que baixaram o app pós clique, a distribuição de IPs está mais uniforme, com a maioria
# dentro de L01. Conforme mencionado, vamos manter esta variável no modelo, com a ressalva de que possivelmente os
# algoritmos que gerem cliques fraudulentos possam estar mascarando o seu IP através de uma VPN, o que pode explicar
# em partes o por que da distribuição de IP's para quem baixou o APP estar com uma distribuição mais uniforme.

# A variável "app" identifica um app pelo ID, assim como a variável "device" identifica o tipo de dispositvo do 
# registro, e a variável "channel" identifica o ID do publisher, vamos converter estas variáveis para char,
# pois no modelo elas podem indicar que um app vem sendo mais alvo, ou dispositivo gerem mais ou menos downloads
# após o clique na propoaganda.
...
# A distribuição de dados por dia da semana é semelhante tanto para todos os registros do dataset,
# quanto para os casos do qual o clique gerou download do app.
...
# O mesmo pode se dizer olhando para o perído do dia, com uma leve diferença no período da tarde e noite, para os casos
# em que o clique no link gerou download no app (houveram menos cliques em relação aos outros períodos do dia)
...
# Precisamos transformar as variáveis do tipo character em fator para posterior balanceamento de classes.
...
# No geral, não houveram grandes modificações na distribuições dos dados após o balanveamento de classes.
# Podemos verificar ta,bém que alguns apps estão mais propoensos a serem baixados após o clique na propaganda, 
# o mesmo vale para os dispositivos.
...
roc.curve(rose_teste$is_attributed, previsoes1, plotit = T, col = "green", add.roc = T)

# Obtemos 84~85% de acurácia, com 94~95% de sensibilidade e 76~77% de especificidade, 
# Já a área sob a curva (AUC) ficou em 84,5%. No geral, para uma primeia versão do modelo
# os resultados podem estão bons, entretanto podemos tentar otimizar o modelo para tentar
# equilibrar os valores de sensibildiade e especificidade, diminuindo assim a taxa de falsos
# positivos e aumentando o Score AUC.


#Otimização do Modelo

# Vamos verificar a importancia de cada variável para o modelo criado, e assim criar um modelo
# apenas com a svariáveis mais importantes.
...
# Pela técnica utilizada, podemos notar que para o modelo criado, as variáveis "app_chr", "channel_chr", 
# "ip_cut" e "device_chr" são as mais importantes, (com mais de 80% de Overall) 
# vamos criar um modelo com elas:
...
# Calculamos o Score AUC
roc.curve(rose_teste$is_attributed, previsoes2, plotit = T, col = "green", add.roc = T)

# Com a seleção de variáveis, conseguimos aumentar a acurácia do modelo de 88~90%, com uma sensibilidade de 94~96%,
# e especificidade de 83~86%, além do aumento do Socre AUC de 88~90%

# Conclusão: Baseado no dataset utilizado, podemos concluir que o modelo 2 criado conseguiria prever com 90%
# de precisão se um app será baixado após o clique na propaganda ou não, os erros do modelo seriam em sua maioria 
# falsos positivos (o modelo acusar que um app será baixado quando na verdade ele não será), além disso, podemos
# concluir que alguns apps possuem mais chances de serem baixados do que outros (após o clique), assim como
# alguns apps possuem menos chances de serem baixados (possam estar sendo alvos de cliques fraudulentos). O mesmo
# vale para o ID do editor e a faixa de IP's. Com uma rápida consulta podemos descobrir quais são exatamente estes 
# apps, dispositivos, etc.
